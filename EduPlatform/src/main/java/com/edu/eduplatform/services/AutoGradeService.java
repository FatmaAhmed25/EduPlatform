...
public class AutoGradeService
{
    ...
    public List<QuestionAnswerDTO> getQuestionAnswersByQuizIdAndStudentIdAndGradeIsNull(Long quizId, Long studentId) {
        EssaySubmission essaySubmission = essaySubmissionRepository.findByQuizQuizIdAndStudentUserIDAndTotalGradeIsNull(quizId, studentId);
        if (essaySubmission == null) {
            throw new IllegalArgumentException("Submission not found for the given quiz and student.");
        }
...
//    public void autoGradeGeneratedEssays(Long courseId,Long quizId,Long studentId,String foldername) throws IOException {
//        List<MultipartFile> pdfFiles=courseContentService.getPdfFiles(courseId.toString(),foldername);
//        essayAutoGrade(quizId,studentId,pdfFiles);
//    }
...
//            System.out.println("Overall Grade: " + overallGrade);
    ...
    public void essayAutoGradeAll(Long quizId, List<Long> studentIds, List<MultipartFile> pdfFiles) {
        // Fetch questions from the database
        List<Question> questions = questionRepository.findByQuizQuizId(quizId);
        List<String> questionTexts = questions.stream().map(Question::getText).collect(Collectors.toList());
        List<Long> questionIds = questions.stream().map(Question::getQuestionId).collect(Collectors.toList());

        System.out.println("Questions: " + questionTexts);
        ...
        for (Long studentId : studentIds) {
            ...
            List<QuestionAnswerDTO> questionAnswers = getQuestionAnswersByQuizIdAndStudentIdAndGradeIsNull(quizId, studentId);
            List<String> userAnswers = questionAnswers.stream().map(QuestionAnswerDTO::getUserAnswer).collect(Collectors.toList());

            System.out.println("User Answers for Student " + studentId + ": " + userAnswers);
            ...
            try {
                ...
                System.out.println("Grades for Student " + studentId + ": " + grades);
                ...
                for (int i = 0; i < questionIds.size(); i++) {
                    System.out.println("Grade: " + grades.get(i));
                    double currentPoints = questionRepository.findById(questionIds.get(i)).get().getPoints();
                    double currentGrade = grades.get(i) * currentPoints;
